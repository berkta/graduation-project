...
for epoch in range(max_epoch):
    ...
    for epoch_index in range(total_samples):            
        ...
        ground_truth_batch = torch.zeros(1, batch_size, 1)  # create tensor of zeros with batch_size as its size to use as ground truth
        ...
        for batch_index in range(batch_size):  # loop for getting random pieces 
            ...
            ground_truth_batch[0, batch_index, 0] = y_gt[rand_index+seq_length]  # assign the same random element of y_gt to ground truth tensor's element 
    ...
    if epoch % test_number == 0:  # put every 'test_number' of epochs to test loop
        ...
        for iteration_index in range(seq_length, total_samples):
            h_state = torch.zeros(num_layers, 1, hidden_size)  # initialize fresh hidden state to use in test loop

            out, h_state = model( x[iteration_index - seq_length + 1 : iteration_index + 1].view(seq_length, 1, -1), h_state)

            out_buffer[iteration_index] = out[0, 0, 0]               
            loss = loss_function(y_noisy[iteration_index], out)
            
            # print epoch, iteration and its loss
            print("Epoch: ", str(epoch), " Iteration: ", str(iteration_index), " Loss: ", str(loss.data))
        # plot graph
        plt.plot(x.view(total_samples).cpu().detach(), y_noisy.view(total_samples).detach())
