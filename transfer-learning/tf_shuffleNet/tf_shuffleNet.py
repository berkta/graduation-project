...
netname = "net"
best_model = "en_iyi_model_shuffleNet" 
...
def train_model(model, criterion, optimizer, scheduler, num_epochs = 25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if epoch % 5 == 4:
                torch.save(model.state_dict(), "./saved_models_shuffleNet/" + netname + "_epoch_%d"%(epoch) + ".pth")
    ...
    elapsedTimeFile = open('elapsed_time_shuffleNet.txt', 'w')  # open a text file in write mode
    elapsedTimeFile.write('ShuffleNetV2_x1_0\n{:.0f}dk {:.0f}s\nEn yüksek val doğruluğu: {:4f}'.format(time_elapsed // 60, 
                                                                                                      time_elapsed % 60, best_acc))  # elapsed time is written on a text file
    elapsedTimeFile.close()  # close the text file

    # load best model weights
    model.load_state_dict(best_model_wts)
    torch.save(best_model_wts, "./saved_models_shuffleNet/" + best_model + ".pth")
    return model
