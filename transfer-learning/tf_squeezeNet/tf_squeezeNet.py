...
netname = "net"
best_model = "en_iyi_model_squeezeNet" 
...
data_dir = r'C:\Users\Mehmet\Desktop\mobileNets\animals'  # data path of dataset
...
def train_model(model, criterion, optimizer, scheduler, num_epochs = 25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if epoch % 5 == 4:
                torch.save(model.state_dict(), "./saved_models_squeezeNet/" + netname + "_epoch_%d"%(epoch) + ".pth")
    ...
    elapsedTimeFile = open('elapsed_time_squeezeNet.txt', 'w')  # open a text file in write mode
    elapsedTimeFile.write('SqueezeNet1_0\n{:.0f}dk {:.0f}s\nEn yüksek val doğruluğu: {:4f}'.format(time_elapsed // 60, 
                                                                                                      time_elapsed % 60, best_acc))  # elapsed time is written on a text file
    elapsedTimeFile.close()  # close the text file

    # load best model weights
    model.load_state_dict(best_model_wts)
    torch.save(best_model_wts, "./saved_models_squeezeNet/" + best_model + ".pth")
    return model
...
model_conv.classifier[1] = nn.Conv2d(512, len(class_names), kernel_size = (1,1), stride = (1,1))
